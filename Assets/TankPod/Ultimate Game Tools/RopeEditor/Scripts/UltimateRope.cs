                ...
                for(int nLink = 0; nLink < node.segmentLinks.Length; nLink++)
                {
                    ...
                    if(tfSegmentStart != null && tfSegmentEnd != null)
                    {
                        if(nLink == 0)
                        {
                            ...
                            node.segmentLinks[nLink].transform.parent      = LockStartEndInZAxis ? tfSegmentStart : this.transform;
                        ...
                        else if(nLink == node.segmentLinks.Length - 1)
                        {
                            ...
                            node.segmentLinks[nLink].transform.parent      = LockStartEndInZAxis ? tfSegmentEnd : this.transform;
                            node.segmentLinks[nLink].rigidbody.isKinematic = LockStartEndInZAxis;
                        }
                    }
			...
			while(fLinearIncrement < 0.0f && nodeExtension.m_nExtensionLinkIn > 0 && nodeExtension.m_nExtensionLinkIn < nodeExtension.segmentLinks.Length - 1 && bLastStep == false)
			{
				...
				if(nodeExtension.m_fExtensionRemainderIn < -fLinkSeparation)
				{
                    ...
                    if(nodeExtension.segmentLinks[nodeExtension.m_nExtensionLinkIn].rigidbody.isKinematic == false)
                    {
                        ...
                        UltimateRopeLink ropeLink = nodeExtension.segmentLinks[nodeExtension.m_nExtensionLinkIn].GetComponent<UltimateRopeLink>();
                        if(ropeLink != null) ropeLink.ExtensibleKinematic = true;
                    }
