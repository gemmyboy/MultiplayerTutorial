// GrassOnMeshGenerator shader helpers.
//
...
void FastSinCosGOMG (float4 val, out float4 s, out float4 c) {
...
fixed4 WaveGrassOMG (inout float4 vertex, float waveAmount, fixed4 color)
{
...
void BillboardTree( inout float4 pos, float2 offset )
{
	pos.xyz += offset.x * _CameraRight.xyz;
	pos.xyz += offset.y * _CameraUp.xyz;
	//pos.xz += offset.x * _CameraRight.xz;
	//pos.xz += offset.y * _CameraUp.xz;
}
...
void WavingGrassOMGVert (inout appdata_full v)
{
	// MeshGrass v.color.a: 1 on top vertices, 0 on bottom vertices
	// _WaveAndDistance.z == 0 for MeshLit
	float waveAmount = v.color.a * _WaveAndDistance.z;

	v.color = WaveGrassOMG (v.vertex, waveAmount, v.color);
}

void WavingGrassOMGBillboardVert (inout appdata_full v)
{
	BillboardGrassOMG (v.vertex, v.tangent.xy);
