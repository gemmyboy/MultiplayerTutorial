...
namespace TerraVol
{
	...
	internal class MarchingCubes
	{
		...
		private bool[] gridColorInterp = new bool[8];
		...
		*/
		...
		{
			if (negativeBorderX) {
				gridColorInterp[1] = false;
				gridColorInterp[2] = false;
				gridColorInterp[6] = false;
				gridColorInterp[5] = false;
			} else if (positiveBorderX) {
				gridColorInterp[0] = false;
				gridColorInterp[3] = false;
				gridColorInterp[7] = false;
				gridColorInterp[4] = false;
			}
			if (negativeBorderY) {
				gridColorInterp[4] = false;
				gridColorInterp[5] = false;
			...
			for (int i=0; i < 8; i++) {
				...
				if (gridColorInterp[i] && (cubeindex & (1 << i)) != 0) {
