    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float4 node_3124 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((o.uv0+node_3124.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
...
/////// Vectors:
                ...
                float4 node_3124 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((i.uv0+node_3124.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_133 = pow((abs((frac((o.uv0+node_2063.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                v.vertex.xyz += (node_133*_BulgeScale*v.normal);
                o.posWorld = mul(_Object2World, v.vertex);
                float3 lightColor = _LightColor0.rgb;
                o.pos = mul(UNITY_MATRIX_MVP, v.vertex);
...
/////// Vectors:
                ...
                float3 _Normals_var = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(i.uv0, _Normals)));
                float4 node_2063 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((i.uv0+node_2063.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float3 normalLocal = normalize(lerp(_Normals_var.rgb,float3(0,0,1),node_133));
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv0 = v.texcoord0;
                o.normalDir = mul(_Object2World, float4(v.normal,0)).xyz;
                float4 node_5447 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((o.uv0+node_5447.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                v.vertex.xyz += (node_133*_BulgeScale*v.normal);
                o.pos = mul(UNITY_MATRIX_MVP, v.vertex);
...
/////// Vectors:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_133 = pow((abs((frac((o.uv0+node_455.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                v.vertex.xyz += (node_133*_BulgeScale*v.normal);
                o.pos = mul(UNITY_MATRIX_MVP, v.vertex);
