...
public class MainFixedCam : MonoBehaviour {
	
	private Camera[] cams;
	private AudioListener[] listeners;
	private ChildFixedCam[] childCams;
	...
	void Start () {
		...
		childCams = GetComponentsInChildren<ChildFixedCam>();
		switcher = mainCamera.GetComponent<CameraSwitcher>();
		...
		foreach(ChildFixedCam l in childCams){
			l.enabled = false;
			l.player = player;
		}
